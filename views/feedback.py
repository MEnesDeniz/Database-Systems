...
def validate_feedback(form):
    ...
    if pointChecker(form_checking_service) == False:
        form.errors["checking_service"] = "Checking Service support must be between 0 and 5."
    else:
...
def update_feedback(id, ticker):
    ...
    if request.method == "GET":
        ...
        cur.close()
        return render_template("feedback_update.html", id = id , ticker=ticker, values = values)
    if request.method == "POST":
        valid = validate_feedback(request.form)
        if not valid:
            return render_template(
